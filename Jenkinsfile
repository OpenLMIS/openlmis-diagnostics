        ...
        stage('Parallel: Sonar analysis and contract tests') {
            parallel {
                ...
                stage('Contract tests') {
                    steps {
                        build job: 'OpenLMIS-diagnostics-contract-test', propagate: true, wait: true
                    }
                    post {
                        failure {
                            slackSend color: 'danger', message: "${env.JOB_NAME} - #${env.BUILD_NUMBER} ${env.STAGE_NAME} FAILED (<${env.BUILD_URL}|Open>)"
            ...
            steps {
                dir('erd') {
                    sh '''#!/bin/bash -xe
                        ...
                        sudo mkdir -p /var/www/html/erd-diagnostics
                        sudo chown -R $USER:$USER /var/www/html/erd-diagnostics
                        ...
                        && wget https://raw.githubusercontent.com/OpenLMIS/openlmis-diagnostics/master/docker-compose.erd-generation.yml -O docker-compose.yml \
                        && (/usr/local/bin/docker-compose up &) \
                        && sleep 90 \
                        && sudo rm /var/www/html/erd-diagnostics/* -rf \
                        && sudo rm -rf output \
                        && mkdir output \
                        && chmod 777 output \
                        && (docker run --rm --network erd_default -v $WORKSPACE/erd/output:/output schemaspy/schemaspy:snapshot -t pgsql -host db -port 5432 -db open_lmis -s diagnostics -u postgres -p p@ssw0rd -I "(data_loaded)|(schema_version)|(jv_.*)" -norows -hq &) \
                        && sleep 30 \
                        && /usr/local/bin/docker-compose down --volumes \
                        && sudo chown -R $USER:$USER output \
                        && mv output/* /var/www/html/erd-diagnostics \
                        && rm erd-diagnostics.zip -f \
                        && pushd /var/www/html/erd-diagnostics \
                        && zip -r $WORKSPACE/erd/erd-diagnostics.zip . \
                        && popd \
                        && rmdir output \
                        && rm .env \
                        && rm docker-compose.yml
                    '''
                    archiveArtifacts artifacts: 'erd-diagnostics.zip'
                }
            }
            post {
                failure {
